...
// 获取注入内容（供其他模块调用）
export function getRPGInjectionContent() {
...
// 检查是否有待注入的内容
export function hasRPGInjectionContent() {
  return !!(extension_settings[extensionName]?.injectionContent?.trim());
}

// 清空注入内容
export function clearRPGInjectionContent() {
  if (extension_settings[extensionName]) {
    extension_settings[extensionName].injectionContent = "";
    $("#smart_memory_rpg_injection_content").val("");
...
jQuery(async () => {
  ...
  $("#test_rpg_json").on("click", function() {
    ...
    $("#smart_memory_rpg_injection_content").val("测试数据：\n" + jsonStr);
  ...
  // 模型选择更改时保存
  $("#smart_memory_rpg_model").on("change", function() {
  ...
  $("#smart_memory_rpg_enabled").on("change", function() {
  ...
  $("#smart_memory_rpg_auto_update").on("change", function() {
    const isChecked = $(this).prop("checked");
  ...
  $("#smart_memory_rpg_depth").on("input", function() {
    $("#smart_memory_rpg_depth_value").text($(this).val());
  });
  
  // 更新间隔滑块
  $("#smart_memory_rpg_update_interval").on("input", function() {
    const value = $(this).val();
    $("#smart_memory_rpg_update_interval_value").text(value);
