...
async function summarizeMessages() {
  ...
  try {
    ...
    if (summary) {
      ...
      // 确保更新到界面 - 使用多种方法确保成功
      setTimeout(() => {
        const injectionTextarea = document.getElementById("smart_memory_rpg_injection_content");
        if (injectionTextarea) {
          ...
          // 也用jQuery更新
          $("#smart_memory_rpg_injection_content").val(summary).trigger('input').trigger('change');
          
          console.log(`智能总结: 注入框已更新，内容长度: ${injectionTextarea.value.length}`);
          
          // 验证是否真的更新了
          const actualValue = $("#smart_memory_rpg_injection_content").val();
          ...
          console.error("智能总结: 找不到注入框元素 #smart_memory_rpg_injection_content");
...
function setupMessageListener() {
  ...
  eventSource.on(event_types.CHAT_CHANGED, () => {
    ...
    if (extension_settings[extensionName]?.characterInjections?.[characterName]) {
      ...
      $("#smart_memory_rpg_injection_content").val(savedContent);
      ...
      // 没有保存的内容，清空
      extension_settings[extensionName].injectionContent = "";
      $("#smart_memory_rpg_injection_content").val("");
...
jQuery(async () => {
  ...
  function createRPGButton() {
    ...
    if ($('#rpg-floating-button').length > 0) {
      console.log('RPG按钮已存在，移除旧按钮');
      $('#rpg-floating-button').remove();
    }
  ...
  // 在设置加载完成后创建按钮，确保正确应用初始状态
  // 不要在这里立即创建按钮，等设置加载完成后再创建
  ...
  // 悬浮按钮开关事件 - 使用事件委托确保即使动态创建也能工作
  $(document).on("change", "#rpg_floating_enabled", function() {
    const isEnabled = $(this).prop("checked");
    extension_settings[extensionName].rpgFloatingEnabled = isEnabled;

    console.log(`RPG悬浮按钮开关: ${isEnabled ? '启用' : '禁用'}`);

    const $button = $('#rpg-floating-button');
    if ($button.length === 0) {
      console.warn('悬浮按钮不存在，无法操作');
      return;
    }

    if (isEnabled) {
      $button.fadeIn(300);
      console.log('✅ RPG悬浮按钮已显示');
    } else {
      $button.fadeOut(300);
      console.log('❌ RPG悬浮按钮已隐藏');

      // 如果窗口打开了，也关闭它
      const $window = $("#rpg-floating-window");
  ...
  // 测试资源服务器按钮
  $("#test_resource_server").on("click", async function() {
    if (window.RPGMemoryGame && window.RPGMemoryGame.testResourceServers) {
      try {
        const results = await window.RPGMemoryGame.testResourceServers();
        const onlineServers = results.filter(r => r.status === 'online');

        if (onlineServers.length > 0) {
          toastr.success(`检测到 ${onlineServers.length} 个可用资源服务器`, "资源服务器测试");
        } else {
          toastr.error("所有资源服务器都不可用", "资源服务器测试");
        }
      } catch (error) {
        toastr.error("测试过程中出错: " + error.message, "资源服务器测试");
      }
    } else {
      toastr.error("RPG游戏模块未加载", "资源服务器测试");
    }
  });
  ...
  $("#smart_memory_rpg_injection_content").on("input change", function() {
  ...
  // 先创建悬浮按钮
  function createRPGButtonNow() {
    createRPGButton(); // 创建按钮

    // 然后根据设置调整显示状态
    setTimeout(() => {
      const isEnabled = extension_settings[extensionName].rpgFloatingEnabled !== false; // 默认为true
      const $checkbox = $("#rpg_floating_enabled");
      const $button = $('#rpg-floating-button');
      ...
      if (!isEnabled) {
        $button.hide();
        console.log('RPG悬浮按钮: 根据设置已隐藏');
      } else {
        $button.show();
        console.log('RPG悬浮按钮: 根据设置已显示');
      }
      ...
      console.log('RPG悬浮按钮状态同步完成:', {
        设置值: extension_settings[extensionName].rpgFloatingEnabled,
        复选框: $checkbox.prop("checked"),
        按钮可见: $button.is(':visible'),
        按钮存在: $button.length > 0
      });
    }, 200); // 增加延迟确保DOM已准备好
  }

  // 多次尝试创建按钮并应用设置，确保成功
  setTimeout(createRPGButtonNow, 500);   // 0.5秒后第一次尝试
  setTimeout(createRPGButtonNow, 1000);  // 1秒后第二次尝试
  setTimeout(createRPGButtonNow, 2000);  // 2秒后第三次尝试
  ...
  // 自动检测资源服务器（延迟执行，避免影响页面加载）
  setTimeout(async () => {
    if (window.RPGMemoryGame && window.RPGMemoryGame.testResourceServers) {
      try {
        console.log("RPG记忆增强器: 开始自动检测资源服务器...");
        const results = await window.RPGMemoryGame.testResourceServers();
        const onlineServers = results.filter(r => r.status === 'online');

        if (onlineServers.length > 0) {
          console.log(`RPG记忆增强器: ✅ 检测到 ${onlineServers.length} 个可用资源服务器`);
        } else {
          console.log("RPG记忆增强器: ❌ 所有资源服务器都不可用，将使用备用资源");
        }
      } catch (error) {
        console.warn("RPG记忆增强器: 自动检测资源服务器时出错:", error);
      }
    }
  }, 3000); // 3秒后开始检测
});
